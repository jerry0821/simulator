#ifndef COMPUTE_WATER_MASK_TEXTURE_H
...
class ComputeWaterMaskTexture : public ComputeTask
{
public:
	ComputeWaterMaskTexture() = default;
	~ComputeWaterMaskTexture() override = default;

	const char* DebugName() const override
	{
		return "ComputeWaterMaskTexture";
	}
	...
	void Update(ID3D11ShaderResourceView* visible_water_srv) const;
	bool IsValid() const override;
	Backend::RenderShaderResource Resource() const;

private:
	struct WaterMaskConstants
	{
		float water_threshold = 0.06f;
		float water_fade = 0.18f;
		float shore_band = 0.24f;
		float padding0 = 0.0f;
...
#endif // COMPUTE_WATER_MASK_TEXTURE_H
