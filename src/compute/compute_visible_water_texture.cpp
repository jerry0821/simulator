...
bool ComputeVisibleWaterTexture::Initialize(ID3D11Device* device, ID3D11DeviceContext* context)
{
	Finalize();
	...
	std::ifstream shader_stream("resource/shader/shader_compute_visible_water.cso", std::ios::binary);
	if (!shader_stream)
	{
		hal::dout << "ComputeVisibleWaterTexture::Initialize(): failed to open shader_compute_visible_water.cso" << std::endl;
		return false;
	}
	...
	{
		hal::dout << "ComputeVisibleWaterTexture::Initialize(): CreateComputeShader failed" << std::endl;
		Finalize();
		return false;
	}
	...
	buffer_desc.ByteWidth = sizeof(VisibleWaterConstants);
...
void ComputeVisibleWaterTexture::Finalize()
{
	...
	ID3D11ShaderResourceView* surface_water_flow_srv) const
{
	...
	const VisibleWaterConstants constants = {
		0.02f,
		0.10f,
		0.22f,
		0.08f,
...
bool ComputeVisibleWaterTexture::IsValid() const
{
...
Backend::RenderShaderResource ComputeVisibleWaterTexture::Resource() const
{
