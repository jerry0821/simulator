...
bool ComputeWaterMaskTexture::Initialize(ID3D11Device* device, ID3D11DeviceContext* context)
{
	Finalize();
	...
	std::ifstream shader_stream("resource/shader/shader_compute_water_mask.cso", std::ios::binary);
	if (!shader_stream)
	{
		hal::dout << "ComputeWaterMaskTexture::Initialize(): failed to open shader_compute_water_mask.cso" << std::endl;
		return false;
	}
	...
	{
		hal::dout << "ComputeWaterMaskTexture::Initialize(): CreateComputeShader failed" << std::endl;
		Finalize();
		return false;
	}
	...
	buffer_desc.ByteWidth = sizeof(WaterMaskConstants);
...
void ComputeWaterMaskTexture::Finalize()
{
...
void ComputeWaterMaskTexture::Update(ID3D11ShaderResourceView* visible_water_srv) const
{
	if (!IsValid() || visible_water_srv == nullptr)
	{
		return;
	}

	const WaterMaskConstants constants = {
		0.06f,
		0.18f,
		0.24f,
		0.0f,
	...
	ID3D11ShaderResourceView* srvs[] = { visible_water_srv };
...
bool ComputeWaterMaskTexture::IsValid() const
{
...
Backend::RenderShaderResource ComputeWaterMaskTexture::Resource() const
{
