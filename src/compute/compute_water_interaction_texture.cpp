...
bool ComputeWaterInteractionTexture::Initialize(ID3D11Device* device, ID3D11DeviceContext* context)
{
	Finalize();
	...
	std::ifstream shader_stream("resource/shader/shader_compute_water_interaction.cso", std::ios::binary);
	if (!shader_stream)
	{
		hal::dout << "ComputeWaterInteractionTexture::Initialize(): failed to open shader_compute_water_interaction.cso" << std::endl;
		return false;
	}
	...
	{
		hal::dout << "ComputeWaterInteractionTexture::Initialize(): CreateComputeShader failed" << std::endl;
		Finalize();
		return false;
	}
	...
	buffer_desc.ByteWidth = sizeof(WaterInteractionConstants);
	buffer_desc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	if (FAILED(m_device->CreateBuffer(&buffer_desc, nullptr, &m_constant_buffer)))
	{
		hal::dout << "ComputeWaterInteractionTexture::Initialize(): CreateBuffer(constant) failed" << std::endl;
		Finalize();
		return false;
	}
...
void ComputeWaterInteractionTexture::Finalize()
{
...
void ComputeWaterInteractionTexture::Update(
	...
	ID3D11ShaderResourceView* soil_moisture_srv)
{
	...
	const WaterInteractionConstants constants = {
		kTextureWidth,
		kTextureHeight,
		0u,
		0u };
...
bool ComputeWaterInteractionTexture::IsValid() const
{
...
Backend::RenderShaderResource ComputeWaterInteractionTexture::Resource() const
{
