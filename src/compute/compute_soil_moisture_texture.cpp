...
bool ComputeSoilMoistureTexture::Initialize(ID3D11Device* device, ID3D11DeviceContext* context)
{
	Finalize();
	...
	std::ifstream shader_stream("resource/shader/shader_compute_soil_moisture.cso", std::ios::binary);
	if (!shader_stream)
	{
		hal::dout << "ComputeSoilMoistureTexture::Initialize(): failed to open shader_compute_soil_moisture.cso" << std::endl;
		return false;
	}
	...
	{
		hal::dout << "ComputeSoilMoistureTexture::Initialize(): CreateComputeShader failed" << std::endl;
		Finalize();
		return false;
	}
	...
	buffer_desc.ByteWidth = sizeof(SoilMoistureConstants);
	buffer_desc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;

	if (FAILED(m_device->CreateBuffer(&buffer_desc, nullptr, &m_constant_buffer)))
	{
		hal::dout << "ComputeSoilMoistureTexture::Initialize(): CreateBuffer(constant) failed" << std::endl;
		Finalize();
		return false;
	}
...
void ComputeSoilMoistureTexture::Finalize()
{
	...
	ID3D11ShaderResourceView* water_mask_srv)
{
	...
	const SoilMoistureConstants constants = {
		0.16f,
		0.14f,
		0.10f,
		0.045f,
...
bool ComputeSoilMoistureTexture::IsValid() const
{
...
Backend::RenderShaderResource ComputeSoilMoistureTexture::Resource() const
{
