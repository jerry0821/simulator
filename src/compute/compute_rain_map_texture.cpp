...
bool ComputeRainMapTexture::Initialize(ID3D11Device* device, ID3D11DeviceContext* context)
{
	Finalize();
	...
	std::ifstream shader_stream("resource/shader/shader_compute_rain_map.cso", std::ios::binary);
	if (!shader_stream)
	{
		hal::dout << "ComputeRainMapTexture::Initialize(): failed to open shader_compute_rain_map.cso" << std::endl;
		return false;
	}
	...
	{
		hal::dout << "ComputeRainMapTexture::Initialize(): CreateComputeShader failed" << std::endl;
		Finalize();
		return false;
	}
	...
	buffer_desc.ByteWidth = sizeof(RainMapConstants);
	buffer_desc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;

	if (FAILED(m_device->CreateBuffer(&buffer_desc, nullptr, &m_constant_buffer)))
	{
		hal::dout << "ComputeRainMapTexture::Initialize(): CreateBuffer(constant) failed" << std::endl;
		Finalize();
		return false;
	}
...
void ComputeRainMapTexture::Finalize()
{
	...
	float time_seconds) const
{
	...
	const RainMapConstants constants = {
		time_seconds,
		0.28f,
		2.15f,
		1.0f,
...
bool ComputeRainMapTexture::IsValid() const
{
...
Backend::RenderShaderResource ComputeRainMapTexture::Resource() const
{
