// ----------------------------------------------------
// 3D sprite cutout shader [shader_sprite3d_cutout.cpp]
// ====================================================
...
ID3D11Buffer* g_vs_constant_buffer3 = nullptr;
...
{
	std::ifstream ifs_vs("resource/shader/shader_vertex_sprite3d_cutout.cso", std::ios::binary);
	if (!ifs_vs)
	{
		MessageBox(nullptr, "sprite3d cutout vertex shader load failed", "Error", MB_OK);
		return false;
	}
	...
	if (FAILED(hr))
	{
		hal::dout << "ShaderSprite3D_Cutout: failed to create VS" << std::endl;
		return false;
	}
	...
	if (FAILED(hr))
	{
		hal::dout << "ShaderSprite3D_Cutout: failed to create input layout" << std::endl;
		return false;
	}
	...
	if (!ifs_ps)
	{
		MessageBox(nullptr, "sprite3d cutout pixel shader load failed", "Error", MB_OK);
		return false;
	}
	...
	if (FAILED(hr))
	{
		hal::dout << "ShaderSprite3D_Cutout: failed to create PS" << std::endl;
		return false;
	}
	...
	device->CreateBuffer(&buffer_desc, nullptr, &g_vs_constant_buffer3);
...
void ShaderSprite3D_Cutout_Finalize()
{
	...
	SAFE_RELEASE(g_vs_constant_buffer3);
...
void ShaderSprite3D_Cutout_SetWorldMatrix(const XMMATRIX& matrix)
{
...
void ShaderSprite3D_Cutout_SetViewMatrix(const XMMATRIX& matrix)
{
	XMFLOAT4X4 transpose{};
	XMStoreFloat4x4(&transpose, XMMatrixTranspose(matrix));
...
void ShaderSprite3D_Cutout_SetProjMatrix(const XMMATRIX& matrix)
{
	XMFLOAT4X4 transpose{};
	XMStoreFloat4x4(&transpose, XMMatrixTranspose(matrix));
	Direct3D_GetContext()->UpdateSubresource(g_vs_constant_buffer2, 0, nullptr, &transpose, 0, 0);
}

void ShaderSprite3D_Cutout_SetMaterialColor(const XMFLOAT4& material_color)
{
										   ...
										   float world_max_z)
{
	...
	Direct3D_GetContext()->UpdateSubresource(g_vs_constant_buffer3, 0, nullptr, &constants, 0, 0);
}
...
void ShaderSprite3D_Cutout_Begin()
{
	...
	context->VSSetConstantBuffers(3, 1, &g_vs_constant_buffer3);
