...
static ID3D11Buffer* g_pVSConstantBuffer0 = nullptr; // world
static ID3D11Buffer* g_pVSConstantBuffer1 = nullptr; // view
static ID3D11Buffer* g_pVSConstantBuffer2 = nullptr; // proj
...
void Shader3D_SetMaterialColor(const XMFLOAT4& color)
{
	//定数バッファに行列をセット
...
void Shader3D_Begin()
{
	...
	ID3D11Buffer* pBuffers[3] = {
		g_pVSConstantBuffer0, // b0: World
		g_pVSConstantBuffer1, // b1: View (之前缺這個)
		g_pVSConstantBuffer2  // b2: Proj (之前缺這個)
	};

	// 從 Slot 0 開始，設定 3 個緩衝區
