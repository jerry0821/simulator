...
bool Shader_Initialize(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
...

	{
		...
		ps_buffer_desc.ByteWidth = sizeof(XMFLOAT4); // float4 (16バイト)
		ps_buffer_desc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;

		hr = g_pDevice->CreateBuffer(&ps_buffer_desc, nullptr, &g_pPSConstantBuffer0);
		if (FAILED(hr)) {
			hal::dout << "Shader_Initialize() : PS定数バッファの作成に失敗しました" << std::endl;
			return false;
		}
	}
...
void Shader_Begin()
{
	...
	if (g_pPSConstantBuffer0)
	{
		...
		g_pContext->UpdateSubresource(g_pPSConstantBuffer0, 0, nullptr, &white, 0, 0);
