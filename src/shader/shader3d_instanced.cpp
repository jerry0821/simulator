...
{
	std::ifstream ifs_vs("resource/shader/shader_vertex_3d_instanced.cso", std::ios::binary);
	if (!ifs_vs)
	{
		MessageBox(nullptr, "instanced vertex shader load failed", "Error", MB_OK);
		return false;
	}
	...
	if (FAILED(hr))
	{
		hal::dout << "Shader3DInstanced: failed to create VS" << std::endl;
		return false;
	}
	...
	if (FAILED(hr))
	{
		hal::dout << "Shader3DInstanced: failed to create input layout" << std::endl;
		return false;
	}
	...
	if (!ifs_ps)
	{
		MessageBox(nullptr, "pixel shader load failed", "Error", MB_OK);
		return false;
	}
	...
	if (FAILED(hr))
	{
		hal::dout << "Shader3DInstanced: failed to create PS" << std::endl;
		return false;
	}
